...
const sendSingleMessage = async (doc, isRest, create) => {
  ...
  return sendMessage('emailVerifierNotification', {
    action: 'emailVerify',
    data: [doc],
  })
}
...
const single = async (email, isRest) => {
  ...
  if (validDomain && validMailbox) {
    return sendSingleMessage(
      { email, status: EMAIL_VALIDATION_STATUSES.VALID },
      isRest,
      true,
    )
  }
  ...
  if (EMAIL_VERIFICATION_TYPE === 'truemail') {
    try {
      ...
      return sendSingleMessage(
        { email, status: EMAIL_VALIDATION_STATUSES.UNKNOWN },
        isRest,
      )
    }
  }
  ...
  return sendSingleMessage(
    { email, status: EMAIL_VALIDATION_STATUSES.INVALID },
    isRest,
  )
}
...
const bulk = async emails => {
  ...
  if (verifiedEmails.length > 0) {
    sendMessage('emailVerifierNotification', {
      action: 'emailVerify',
      data: verifiedEmails,
    })
  }
